...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        eleicao = orm['eleicao.Eleicao'].objects.first()
        for veiculo in orm.Veiculo.objects.filter(motorista_titulo_eleitoral__isnull=False).exclude(motorista_titulo_eleitoral='000000000000'):
            if orm['core.Pessoa'].objects.filter(titulo_eleitoral__iexact=veiculo.motorista_titulo_eleitoral).exists():
                print veiculo.pk, veiculo.motorista_titulo_eleitoral
                continue
            pessoa = orm['core.Pessoa'].objects.create(titulo_eleitoral=veiculo.motorista_titulo_eleitoral,
                                                       nome=veiculo.motorista_nome, endereco=veiculo.endereco)
            if veiculo.tel_celular:
                pessoa.telefones_set.create(numero=veiculo.tel_celular, tipo=2)
            if veiculo.tel_residencial:
                pessoa.telefones_set.create(numero=veiculo.tel_residencial, tipo=0)

            motorista = orm.Motorista.objects.create(pessoa=pessoa, veiculo=veiculo, eleicao=eleicao)
    ...
    models = {
        ...
        u'veiculos.motorista': {
            'Meta': {'unique_together': "(('pessoa', 'eleicao'),)", 'object_name': 'Motorista'},
            'eleicao': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'motorista_eleicao'", 'to': u"orm['eleicao.Eleicao']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'pessoa': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['core.Pessoa']"}),
            'veiculo': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'motorista_veiculo'", 'to': u"orm['veiculos.Veiculo']"})
        },
