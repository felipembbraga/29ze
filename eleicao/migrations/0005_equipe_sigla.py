...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        import re
        RE = re.compile(r'^\D+(?P<num>\d+)$')
        for equipe in orm.Equipe.objects.all():
            busca = RE.search(equipe.nome)
            if busca:
                equipe.sigla = busca.group('num')
            else:
                equipe.sigla = len(equipe.nome)<= 4 and equipe.nome or equipe.nome[:3]
            equipe.save()
